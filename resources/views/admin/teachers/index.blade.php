...
@section('content')
    ...
    <div class="page-header d-flex align-items-start justify-content-between flex-wrap gap-3">
        <div>
            <h1 class="page-title">{{ __('messages.teachers_title') }}</h1>
            <p class="page-sub">{{ __('messages.manage_teachers_desc') }}</p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            ...
            <a href="{{ route('admin.teachers.create') }}" class="btn-primary-sm"><i class="bi bi-plus-circle"></i>
                {{ __('messages.add_teacher') }}</a>
        </div>
    </div>
    ...
    <div class="collapse mb-3" id="import-panel">
        <div class="panel-card">
            <div class="panel-card-header">
                <h2 class="panel-card-title"><i class="bi bi-file-earmark-arrow-up"></i> استيراد المعلمين من Excel</h2>
            </div>
            <div class="panel-card-body">
                <p class="text-muted small mb-2">الأعمدة المطلوبة: <strong>الاسم</strong> (إلزامي)، البريد_الإلكتروني،
                    الهاتف، كلمة_المرور (افتراضي: Pass@1234)</p>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    @if ($teacher->avatar)
                                        <img src="{{ asset('assets/uploads/teachers/' . $teacher->avatar) }}"
                                            class="avatar avatar-sm" alt="">
                                    @else
                                        <div class="avatar avatar-sm" style="background:#ecfdf5;color:#059669">
                                            {{ strtoupper(substr($teacher->name, 0, 1)) }}</div>
                                    @endif
                                    <div>
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="{{ route('admin.teachers.show', $teacher->id) }}" class="btn-outline-sm"
                                        style="padding:4px 8px"><i class="bi bi-eye"></i></a>
                                    <a href="{{ route('admin.teachers.edit', $teacher->id) }}" class="btn-outline-sm"
                                        style="padding:4px 8px"><i class="bi bi-pencil"></i></a>
                                    <form action="{{ route('admin.teachers.destroy', $teacher->id) }}" method="POST"
                                        onsubmit="return confirm('{{ __('messages.delete_teacher_confirm') }}')">
                                        @csrf @method('DELETE')
