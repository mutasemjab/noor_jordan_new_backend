...
class WorksheetController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $paginated = Worksheet::with(['subject', 'schoolClass'])
    ...
    // PUT /worksheets/{worksheet}  (mobile sends POST + _method=PUT)
    public function update(Request $request, Worksheet $worksheet): JsonResponse
    {
        $teacher = $request->user();

        if (! $this->owns($teacher, $worksheet)) {
        ...
        return response()->json([
            'status'  => true,
            'message' => 'OK',
            'data'    => $this->itemCard($worksheet->fresh(['subject', 'schoolClass'])),
        ]);
    }

    // DELETE /worksheets/{worksheet}
    public function destroy(Request $request, Worksheet $worksheet): JsonResponse
    {
        $teacher = $request->user();
    ...
    private function owns(Teacher $teacher, Worksheet $item): bool
    {
        return (int) $item->teacher_id === (int) $teacher->id;
    }

    private function itemCard(Worksheet $item): array
    {
        return [
            'id'        => $item->id,
            'title'     => $item->title_ar,
